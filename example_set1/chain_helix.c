   ...
   while(!feof(fp1)) 
   {
      ...
      if(strcmp(temp_code,code))
      {
        ...
        /* strcat(pdbfile,"pdb"); */
         ...
         strcat(pdbfile,".pdb");    /*# taushif modified */
   ...
   while(!feof(fptr))
   {
      ...
      if(line[13]!='!')
      {
         ...
         if(line[11]==code[4])
         {
            ...
            {
               residue_sec_str_type[i]='U'; /*modified 14_4_!4 by taushif*/
            }
...
struct DISTANCE** residue_distance(int helix1, int helix2, struct HELIX *helix, struct HELIXPAIR **helix_pair)
{
   ...
   if(i!=j)
   {
      ...
      if((residue_residue[m][n].distance<=15.0) && (helix[i].chain[4]==helix[j].chain[4])) 
      {
         for(g=0; g<helix[i].residues_total; g++)
         {
            for(h=0; h<helix[j].residues_total; h++)
